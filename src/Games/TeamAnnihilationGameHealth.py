...
class TeamAnnihilationGameClass(GameClass):
    """
    Class to define a Annihilation game between teams of agents.
    Here, the game concludes when all units are eliminated
    ...
    def __init__(self, Agents):
        ...
        self.FiniteLifetimeUnits = ( ProjectileClass, ) # Implemented such that a projectile can only travel a fixed number of cells
...
#                PreviousNewUnit.Position = tuple(PreviousPosition)
        ...
        for UnitID1, Unit1 in State.Units.items():
            if isinstance(Unit1,FlagClass) == False and Unit1.Health != 0 and any(isinstance(Unit1,T) for T in self.InfiniteLifeUnits)==False:
                for UnitID2, Unit2 in State.Units.items():
                    if isinstance(Unit2,FlagClass) == False and Unit2.Health != 0 :
                        ...
                        if Unit1 != Unit2 and (Unit1.overlaps(PreviousUnit2) and Unit2.overlaps(PreviousUnit1)) and any(isinstance(Unit2,T) for T in self.InfiniteLifeUnits)==False:
        ...
        for UnitID in RemoveUnits:
            if any([ isinstance(State.Units[UnitID], T) for T in self.FiniteLifetimeUnits ]):
                State.Units.pop(UnitID)
            else:
        ...
        for UnitID1, Unit1 in State.Units.items():
            if any(isinstance(Unit1,T) == False for T in self.InfiniteLifeUnits) and Unit1.Health != 0:
                for UnitID2, Unit2 in State.Units.items():
                    if any(isinstance(Unit2,T) == False for T in self.InfiniteLifeUnits) and Unit2.Health != 0:
                        if Unit1 != Unit2 and Unit1.overlaps(Unit2):
                                if Unit1.Attack == Unit2.Attack: # Both units are either attacking or not
                                    ...
                                    if UnitID1 not in RemoveUnits:
                                        ...
                                        print(msg)
                                    RemoveUnits.add(UnitID1)
                    elif Unit1.overlaps(Unit2):
                        msg += f"Removing {type(Unit1)} of Agent {Unit1.Owner} for running into a {type(Unit2)}"
                        print(msg)
                        RemoveUnits.add(UnitID1)                        
        ...
        for UnitID, Unit in State.Units.items():
            if any([ isinstance(Unit,T) for T in self.FiniteLifetimeUnits]):
        ...
        for Observer in State.Units.values():
            ...
            for (UnitID, Unit) in State.Units.items():
                ...
                if NewUnit is not None:
                    ...
                    if ObservedStates[AgentID][UnitID] != {NewUnit} and NewUnit is not None:
